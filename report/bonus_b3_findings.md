...
Arbitrator: **Llama 3.1-8B (llama3.1:8b via Ollama, local inference)**. Edge cases = the two primary models (XGBoost-hybrid and the 1D-CNN) disagree (score gap ≥ 0.4) or either is unconfident (score in [0.35, 0.65]). Reproduce with `python analysis/bonus_b3_llm_eval.py --ollama` (`results/bonus_b3_edge_eval.json`).
...
| **pooled** | **4964** | **704** | **14.2%** | **0.618** | **0.679** | — | 11.24 | **704** |
