**Figure 7.1 — Three-stage cascade detector.** Raw command → Isolation Forest bulk-benign filter (stage 1) → XGBoost-hybrid classifier (stage 2: P(attack) > 0.7 → attack, < 0.3 → benign) → LLM arbitration (Llama 3.1-8B) on the 0.3–0.7 edge band.
