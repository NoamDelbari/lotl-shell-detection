...
per docs/superpowers/specs/2026-08-08-featurize-redesign-design.md; pruned
2026-08-09 to the 43 features surviving the joint audit verdicts (Noam +
Claude). Per-feature empirical evidence and KEEP/KILL verdicts:
...
# by the n_sensitive_paths lump + n_redirect_out).
...
# Legacy lumped list -- KEPT by the joint audit: its themed family-C splits
# (n_cred/n_proc/n_log_paths) each failed the effect gate alone, while the
# lump aggregates them into real signal (see report/ch3_feature_decisions.md).
...
# Authoritative column order. The 43 survivors of the joint 68-candidate
# audit (verdicts: report/ch3_feature_decisions.md); every entry carries its
...
FEATURE_NAMES = [
    ...
    "has_shell_bin",         # shell binary named (P8 audit-flagged)
    ...
    "n_sensitive_paths",     # sensitive-path lump (beat its themed splits)
...
def _features_for_one(s: str) -> dict:
    ...
    head, _ = _head_and_prefix(tokens_ws)
