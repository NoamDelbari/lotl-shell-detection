...
|---|---|---|---|---|
| `xgboost`, `xgboost_hybrid` | Boosting | 43 features; hybrid adds 3,000 char n-grams | **A linear or distance-based learner on the same vector.** Axis-aligned splits learn conjunctive threshold rules on mixed scales and Ch. 3's heavy tails, where a linear model is unbounded; gain keeps the surface auditable. | Chen & Guestrin, KDD 2016; shell-command GBDTs, Trizna et al., ACM TOPS 2026 |
...
**Four paradigms is not four representations.** Four of the five read the same
43 features, leaving the CNN alone on that axis; every supervised model, the CNN
included, is fitted on the same attack corpus, free to learn its idiom rather
than its behaviour. Only the Isolation Forest, which reads no attack label,
escapes that. **The project's
weakest standalone detector (Ch. 8) is carried anyway**, because its errors are
decorrelated by construction — what the cascade's first stage requires.
