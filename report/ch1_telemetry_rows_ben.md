...
| **Derived / engineered feature(s)** | `has_fetch_bin`, `has_url`, `has_ipv4`, `has_private_ip`, `n_pipes`, `has_shell_bin` (co-occurrence of a fetch tool + a pipe + a shell is the signature). |
...
| **Derived / engineered feature(s)** | `has_dev_tcp`, `has_shell_flag_i`, `has_exec_flag`, `n_redirect_out`, `has_ipv4`, `has_shell_bin`/`has_interp_bin`. |
| **Explanation** | `/dev/tcp` has essentially no legitimate interactive use — its appearance is a near-categorical malicious signal, captured by `has_dev_tcp`. The interactive/exec flags and the redirect count separate a *shell being wired to a socket* from ordinary shell invocation. `n_redirect_out` is deliberately a count, not a boolean, because reverse shells stack several fd redirections (`0<&196;exec 196<>/dev/tcp/..;sh <&196 >&196`) — a level ordinary commands rarely reach, which the Ch3 effect size confirms (rank-biserial +0.229). |
...
| **Derived / engineered feature(s)** | `has_lotl_bin`, `head_is_lotl`, `n_quotes`, `n_sensitive_paths`, `special_ratio`, `max_token_len`. |
| **Explanation** | The binary itself is benign, so a name-only rule fails; maliciousness lives in the *argument structure* — a normal tool invoked with a shell-spawning payload. `has_lotl_bin`/`head_is_lotl` flag the trusted binary; the quoted payload raises `n_quotes`, `special_ratio` and `max_token_len` (`'BEGIN{system("/bin/sh")}'` is one long, operator-dense token); `n_sensitive_paths` captures the protected targets these escapes typically read. Ch4 partly bears this out and partly qualifies it: `head_is_lotl` is a top-4 gain feature on **both** corpora (0.045 / 0.066), but `has_lotl_bin` and `n_sensitive_paths` are only top-third on Dataset 1 (consensus 7th and 13th of 43) and fall to mid-pack on Dataset 2 (21st and 18th) — the intent-bearing features matter less than the structural ones, which is the discrepancy §4.2 examines. |
