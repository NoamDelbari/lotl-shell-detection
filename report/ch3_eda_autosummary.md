...
- **41 / 43** features differ between classes at p < 0.05.
...
|---|---:|---:|---:|---:|
| `n_abs_paths` | 0.7134 | 0.0977 | +0.452 | 0.00e+00 |
...
| `n_redirect_out` | 0.3148 | 0.0555 | +0.229 | 5.72e-273 |
| `n_sensitive_paths` | 0.2564 | 0.0226 | +0.182 | 5.17e-272 |
| `has_shell_bin` | 0.1885 | 0.007 | +0.181 | 0.00e+00 |
| `n_pipes` | 0.3111 | 0.5567 | -0.166 | 7.79e-69 |
| `mean_token_len` | 7.2652 | 6.2483 | +0.161 | 1.87e-40 |

### 3.2 Noise & redundancy reduction

- **Low-signal features** (|r| < 0.05, candidates to prune for compute): `head_is_lotl`, `has_stderr_merge`, `has_shell_flag_i`, `has_exec_flag`, `head_is_shell`, `has_enum_bin`, `has_dev_tcp`, `has_interp_bin`, `has_hidden_path`, `has_url`, `head_is_interp`, `has_heredoc`, `head_is_privesc`, `has_long_flag`, `has_evasion_tok`, `has_home_ref`, `has_pipe_to_shell`, `has_hex_escape`, `has_decode_exec`, `has_fetch_exec_chain`, `has_quote_splice`, `has_ifs_expansion`, `has_private_ip`.
- **Redundant / multicollinear pairs** (|corr| > 0.85) — keep one of each pair; the tree ranking in Ch4 confirms which:
  ...
  - `has_lotl_bin` ~ `head_is_lotl` (corr 0.871)
